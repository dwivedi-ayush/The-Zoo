...
def BERTsum(orignal_txt):
    def generate_summary_bert(original_text):
        ...
        summary_tokens = tokenizer.decode(inputs["input_ids"].squeeze()[:50].tolist())  # Adjust the number of tokens for the summary
...
def transformers(orignal_txt):
    def generate_summary_t5(original_text):
        ...
        summary = summarizer(original_text, max_length=2048, min_length=50, length_penalty=2.0, num_beams=4)
